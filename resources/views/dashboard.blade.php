                                    ...
                                    <tbody x-data="{ openReview: false }">
                                        <tr class="border-b border-gray-700 hover:bg-gray-700/50 transition">
                                                ...
                                                <div class="font-bold text-lg text-white flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div class="text-gray-400 text-sm mt-1 font-normal">
                                                            Тариф: <span class="text-indigo-400 font-semibold">{{ $server->tariff->name }}</span>
                                            ...
                                            <td class="p-4" x-data="{ copied: false }">
                                                <button @click="navigator.clipboard.writeText('{{ $server->ip_address }}'); copied = true; setTimeout(() => copied = false, 2000)"
                                                    ...
                                                    <span x-show="copied" x-cloak class="absolute -top-8 left-1/2 -translate-x-1/2 bg-green-500 text-white text-xs px-2 py-1 rounded shadow">Скопировано!</span>
                                                </button>
                                            </td>

                                            <td class="p-4">
                                                <span class="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider flex w-max items-center gap-1.5
                                                    ...
                                                    <span class="w-2 h-2 rounded-full {{ $server->status == 'Активен' ? 'bg-green-400 animate-pulse' : 'bg-red-400' }}"></span>
                                                    {{ $server->status }}
                                                </span>
                                            </td>
                                        ...
                                        <tr x-show="openReview" x-cloak class="bg-gray-800/50 border-b border-gray-700">
                                            <td colspan="5" class="p-6">
                                                <form action="{{ route('server.review.store', $server->id) }}" method="POST" class="max-w-3xl bg-gray-900 p-6 rounded-xl shadow-lg border border-gray-700 mx-auto">
                                                    @csrf
                                                    ...
                                                    <textarea name="text" rows="3" class="w-full bg-gray-800 border-gray-700 text-white rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm mb-4 placeholder-gray-500" placeholder="Как вам пинг, стабильность и работа техподдержки?" required></textarea>
                                                    ...
                                                    <div class="flex flex-col sm:flex-row items-center gap-4 justify-between">
                                                        <div class="flex items-center gap-3">
                                                            ...
                                                            <select name="rating" class="bg-gray-800 border-gray-700 text-white rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                                                        ...
                                                        <button type="submit" class="w-full sm:w-auto bg-indigo-600 text-white px-6 py-2.5 rounded-lg text-sm font-bold hover:bg-indigo-500 shadow-lg shadow-indigo-500/30 transition">
                                                            Отправить отзыв
                                                        </button>
                                                    </div>
                                                </form>
